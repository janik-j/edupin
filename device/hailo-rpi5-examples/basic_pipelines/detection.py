...
class user_app_callback_class(app_callback_class):
    def __init__(self):
        ...
        self.min_bbox_size = 0.00  # 5% of frame size
        ...
        # First navigate to hailo root directory, then to lightberry-client/function_calls
        self.target_dir = "../../lightberry-client/function_calls"  # Updated path
...
def copy_and_remove_audio(audio_file, audio_dir, target_dir):
    """Simple function to copy audio file and remove after 5 seconds"""
    try:
        source = os.path.join(audio_dir, audio_file)
        target = os.path.join(target_dir, audio_file)
        
        # Copy file
        shutil.copy2(source, target)
        print(f"Copied {audio_file} to target folder")
        ...
        def remove():
            ...
            if os.path.exists(target):
                os.remove(target)
                print(f"Removed {audio_file} from target folder")
...
def app_callback(pad, info, user_data):
    ...
    for detection in detections:
        ...
        # Check specific objects and trigger audio files
        if label == "green light" and user_data.should_trigger(label, bbox, width, height):
            copy_and_remove_audio("green_signal.mp3", user_data.audio_dir, user_data.target_dir)
            
        elif label == "red light" and user_data.should_trigger(label, bbox, width, height):
            copy_and_remove_audio("red_signal.mp3", user_data.audio_dir, user_data.target_dir)
            
        elif label == "crosswalk" and user_data.should_trigger(label, bbox, width, height):
            copy_and_remove_audio("crosswalk.mp3", user_data.audio_dir, user_data.target_dir)
